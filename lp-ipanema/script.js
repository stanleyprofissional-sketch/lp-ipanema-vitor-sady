        ...
        leadForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    openQuizModal();
                    leadForm.reset();
                    if (typeof fbq === 'function') window.fbq('track', 'Lead'); // Facebook Pixel
                    window.dataLayer = window.dataLayer || [];
                    window.dataLayer.push({ event: 'generate_lead', form_name: 'lead-ipanema' }); // GTM
                } else {
        ...
        quizForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    showQuizSuccess();
                    if (typeof fbq === 'function') window.fbq('track', 'CompleteRegistration'); // Facebook Pixel
                    window.dataLayer = window.dataLayer || [];
                    window.dataLayer.push({ event: 'complete_registration', form_name: 'qualification-ipanema' }); // GTM
...
function trackScrollDepth() {
    ...
    thresholds.forEach(pct => {
        ...
        marker.style.cssText = 'position:absolute;height:1px;width:1px;opacity:0;pointer-events:none;z-index:-1;';
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (!tracked.has(pct)) {
                    ...
                    // GTM
                    window.dataLayer = window.dataLayer || [];
                    window.dataLayer.push({ event: 'scroll_depth', percentage: pct });
                }
            }
        });
    }, { threshold: 0.1 }); // Trigger when at least 10% of the marker is visible

    // Observe markers
    document.querySelectorAll('[data-scroll-depth]').forEach(marker => {
